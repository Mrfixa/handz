...
| M3 | Low (product) | Backend / mart pricing | Order total = subtotal − promo + tip only; no delivery fee / tax / service charge. **Open — genuine product/pricing decision:** needs the business to set rates + an admin config + `mart_orders` columns + migration. Not a defect; implementing arbitrary fees blind would be wrong. Left tracked for when rates are decided. | open | — |
| M4 | Medium | Both apps / mart | Backend sends mart push notifications; the apps don't route them to the tracking screen in-UI. **Open — device-verified follow-up:** push routing/tap-handling is runtime behavior CI can't verify; grouped with the W4-class items that need an on-device pass rather than a blind change to the notification handler. | open | — |
