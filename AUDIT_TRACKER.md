...
| C3 | Med | CI / iOS | After C2 the **user** app reached the Xcode build but both apps hit `connectivity_plus 7.2.0` using `NWPath.isUltraConstrained` (iOS 18 SDK only) — the macos-14 Xcode was too old. Moved the runner to `macos-15` (Xcode 16 / iOS 18 SDK). | fixed | _this batch_ |
| C4 | Med | CI / iOS (driver) | Driver `pod install` can't resolve ML Kit: `google_mlkit_commons` needs `MLKitVision ~>10` while `mobile_scanner`→`GoogleMLKit/BarcodeScanning 7.0`→`MLKitVision ~>8` (non-overlapping). Doesn't affect Android. Fix needs ML Kit plugin-version realignment in pubspec + full Android re-test — deferred (see IOS_BUILD.md). User-app iOS build proves the pipeline. | accepted | — |
